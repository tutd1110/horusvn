...
/**
 * Petition Register Request
*/
class PetitionRegisterRequest extends ApiBaseRequest
{
    /**
    ...
    public function rules()
    {
        ...
        */
        ...
        //block petition > 2 days
        $user_id = auth()->id();
        $super_user_roles = config('const.super_admin');
        $start_date = \Carbon\Carbon::parse($input_data['start_date']);
        $currentTime = \Carbon\Carbon::now();
        $hoursDifference = $currentTime->diffInHours($start_date);
        $hourCompare = !$start_date->isSaturday() && !$start_date->isFriday() ? 72 : 96; // h
        
        $validate_start_date = function ($attribute, $value, $fail) use ($user_id,$super_user_roles,$hoursDifference,$currentTime,$start_date,$hourCompare) {
            $isDayPast = $currentTime->gt($start_date);
            
            if($isDayPast && !in_array($user_id,$super_user_roles) && $hoursDifference > $hourCompare){
                $fail(__('MSG-E-028'));
            }
        };

        $rules += array('start_date' => ['bail', 'required', 'date',$validate_start_date]);
        ...
        if (isset($input_data['type_off']) && $input_data['type_off'] == 4) {
            ...
            *   $fail     : method to call on failure
            */
            $validate_end_date = function ($attribute, $value, $fail) {
                //get Input
                $input_data = $this->all();
            ...
            $rules += array('end_date' => ['bail', 'required', 'date', $validate_end_date]);
        }
