<template>
    ...
    <div id="filter-block">
        <el-row :gutter="10" style="margin-bottom: 15px;">
            <el-col :span="3" style="padding-top: 21px">
                <el-badge :value="totalDAssigned" class="item" :hidden="totalDAssigned === 0">
                    <!-- <el-button style="width: 100%" type="success" v-on:click="onRedirect()">Goto Department Assigned</el-button> -->
                    <el-button style="width: 100%" type="success" v-on:click="onRedirect()">Bug Bộ Phận Bị Gán</el-button>
                </el-badge>
            </el-col>
            <el-col :span="2" style="padding-top: 21px">
                <el-button style="width: 100%" color="#626aef" v-on:click="onStoreAction(0)">Report Bug</el-button>
            </el-col>
            <el-col :span="2" style="padding-top: 21px">
                <el-dropdown style="width: 100%">
                    <el-button style="width: 100%" color="#626aef">
                        Add Feedback
                    </el-button>
                    <template #dropdown>
                        <el-dropdown-menu>
                            <el-dropdown-item @click="onStoreAction(1)">Feedback</el-dropdown-item>
                            <el-dropdown-item @click="onStoreAction(2)">Bổ sung Design</el-dropdown-item>
                            <el-dropdown-item @click="onStoreAction(3)">Sửa Design</el-dropdown-item>
                        </el-dropdown-menu>
                    </template>
                </el-dropdown>
            </el-col>
            <el-col :span="1" style="padding-top: 21px">
                <el-button style="width: 100%" type="warning" :loading="exportLoading" v-on:click="doExport()">Export</el-button>
            </el-col>
            <div class="flex-grow" />
            <el-col :span="5">
        ...
        <el-row :gutter="10" style="margin-bottom: 0;">
            <el-col :span="3">
                <label class="sub-select">Time</label>
                <el-date-picker
                    id="select-daterange"
                    v-model="datePeriod"
                    type="daterange"
            ...
            <el-col :span="2">
                <label class="sub-select">Level</label>
                <el-select
        ...
        <el-row :gutter="10" style="margin-bottom: 0;">
            ...
            <el-col :span="2">
                <label class="sub-select">Tester</label>
                <el-select
                    v-model="formState.tester_id"
                    filterable
                    clearable
                    style="width:100%;"
                >
    ...
    <div class="table-task">
        <el-row :gutter="2" class='table-task-header'>
            <template v-for="(column, idx1) in columns" :key="idx1">
                <el-col :span="column.span" :class="column.class" v-if="column.sorter"  @click="onColClick(column.key)">
                ...
                <el-row :gutter="2" class="table-task-body">
                    <el-col :span="getColSpan('id')">
                        <el-card class="box-card name-task" shadow="hover" style="text-align: center;">
                            <el-popconfirm
                                width="400"
                                v-if="getEditable(record)"
                                confirm-button-text="OK"
                                cancel-button-text="No, Thanks"
                                :icon="InfoFilled"
                                icon-color="#626AEF"
                                title="Are you sure to delete this issue?"
                                @confirm="onClickDeleteButton(record)"
                            >
                                <template #reference>
                                    <Delete
                                        style="width: 18px; cursor: pointer; color:red; margin-right: 8px;"
                                    />
                                </template>
                            </el-popconfirm>
                            <span style="margin-right: 8px;">{{ record.id }}</span>
                            <DocumentCopy
                    ...
                    <el-col :span="getColSpan('start_date')" :class="getColClass('start_date')">
                        <el-card class="box-card" shadow="hover" style="text-align: center;">
                            <el-date-picker
                                ...
                                class="none-border"
                                style="width: 100%;"
                                @change="onChangeSelect(record, 'start_date', $event)"
                            />
                        </el-card>
                    </el-col>
                    <el-col :span="getColSpan('tester_id')">
                        <el-card class="box-card" shadow="hover" style="text-align: center;">
                            <el-select
                                v-model="record.tester_id"
                                filterable
                                clearable
                                :disabled="!getEditable(record)"
                                class="none-border"
                                style="width:100%;"
                                @change="onChangeSelect(record, 'tester_id', $event)"
                            >
                                <el-option
                    ...
                    <el-col :span="getColSpan('assigned_department_id')" :class="getColClass('assigned_department_id')">
                        <el-card class="box-card" shadow="hover" style="text-align: center;">
                            <el-select
                                v-model="record.assigned_department_id"
                                filterable
                                placeholder=""
                                :disabled="!getEditable(record) && !session?.subLeader"
                                class="none-border"
                                clearable
                                style="width:100%;"
                                @change="onChangeSelect(record, 'assigned_department_id', $event)"
                            >
                                <el-option
                                    v-for="item in departments"
                                    :key="item.value"
                    ...
                    <el-col :span="getColSpan('description')" :class="getColClass('description')">
                        <el-card class="box-card" shadow="hover">
                            <el-col :span="22">
                                <el-input
                    ...
                    <el-col :span="getColSpan('assigned_user_id')">
                        <el-card class="box-card" shadow="hover" style="text-align: center;">
                            <el-select
                                ...
                                filterable
                                clearable
                                :disabled="!getEditable(record) && !getEditFixed(record) && !session?.subLeader"
                                class="none-border"
                                style="width:100%;"
                    ...
                    <el-col :span="getColSpan('tag_test')" :class="getColClass('tag_test')">
                        <el-card class="box-card" shadow="hover">
                            <el-select 
...
import { DCaret, CaretBottom, CaretTop, DocumentCopy, ChatDotRound, InfoFilled, Delete } from '@element-plus/icons-vue';
...
const computedFormState = computed<FormState>(() => {
    ...
    if (formState.value.assigned_department_id && formState.value.assigned_department_id.length == 0) {
        newFormState.assigned_department_id = undefined;
    }

    if (formState.value.description) {
        newFormState.description = formState.value.description.split(',');
    }

    return newFormState;
});
const totalDAssigned = ref(0)
...
const clonedUsers = ref<Option[]>([])
const session = ref<Session>()
const projectGetTask = ref([])
const testers = computed(() => {
    return users.value;
    // if (session.value && ([51,107].includes(session.value.id) || session.value.department_id === 5)) {
    //     return users.value;
    // }

    // if (session.value && session.value.department_id) {
    //     return users.value.filter((user: Option) => user.department_id === session.value?.department_id);
    // } else {
    //     return [];
    // }
});
...
const CreateSelbox = () => {
    //create select boxes
    axios.get('/api/task_assignments/dsc-selbox')
    .then(response => {
        ...
        totalDAssigned.value = response.data.count_d_assigned;
        ...
        clonedUsers.value = response.data.users;
        session.value = response.data.session;
    })
}
const dataSource = ref<Item[]>([])
const onChangePage = (page: number) => {
    computedFormState.value.current_page = page

    search()
}
const columns = ref([
    { title: "ID", key: "id", span: 1 },
    { title: "Start", key: "start_date", span: 2, sorter: true, class:"col-custom-1-5"},
    { title: "Tester", key: "tester_id", span: 2 },
    { title: "Project", key: "project_id", span: 2 },
    { title: "Department", key: "assigned_department_id", span: 1, class:"col-custom-1-5"},
    { title: "Task", key: "task_id", span: 2 },
    { title: "Code", key: "task_id_input", span: 1 },
    { title: "Level", key: "level", span: 1 },
    { title: "Weight", key: "weight", span: 1 },
    { title: "Description", key: "description", span: 5, class:"col-custom-5-5"},
    { title: "Fixer", key: "assigned_user_id", span: 2 },
    { title: "Status", key: "status", span: 1 },
    { title: "Tag Test", key: "tag_test", span: 2, class:"col-custom-1-5" },
...
const getColClass = (key: string) => {
    const foundColumn = columns.value.find(column => column.key === key);
    return foundColumn ? foundColumn.class : ''; // Default to 1 if not found
}
...
const sortData = () => {
    if (iconValue.value === 2) {
        computedFormState.value.column = keyColumnClick.value
        computedFormState.value.order = 'asc'
    } else if (iconValue.value === 3) {
        computedFormState.value.column = keyColumnClick.value
        computedFormState.value.order = 'desc'
    } else {
        computedFormState.value.column = undefined
        computedFormState.value.order = undefined
    }

    search()
};
const onStoreAction = (action: number) => {
    axios.post('/api/task_assignments/store', {
        type: action,
...
const getTasksForItem = (item: Item): Option[] => {
    const cacheKey = `${item.project_get_task ? item.project_get_task.join('_') : ''}_${item.assigned_department_id}`;

    return tasksCache[cacheKey] || tasksCache['initial'];
};
const onVisibleChangeTask = (item: Item, is_appear: boolean) => {
    if (is_appear && (item.project_id || projectGetTask.value.length > 0) && item.assigned_department_id) {
...
const onVisibleChangeUser = (item: Item, is_appear: boolean) => {
    if (is_appear) {
        clonedUsers.value = users.value.filter(user => user.department_id === item.assigned_department_id);
    } else {
        clonedUsers.value = users.value
    }
}
...
const showEditTaskTimingModal = (item: Item) => {
    modalTaskTimingRef.value.ShowWithTaskTimingMode(item.id, 'task_assignments', item.assigned_department_id);
}
const showEditNoteModal = (id: number) => {
    modalNoteRef.value.ShowWithNoteMode(id);
}
const showReportModal = () => {
    modalReportRef.value.ShowWithReportMode(computedFormState.value, 'dsc-total');
}
const search = () => {
    openLoading('custom-scrollbar'); // Open the loading indicator before loading data
    
    axios.post('/api/task_assignments/dsc-issues', computedFormState.value)
    .then(response => {
...
const getEditable = (item: Item) => {
    return session.value?.id === item.tester_id || session.value?.editable || session.value?.id == 127
}
const getEditFixed = (item: Item) => {
    return session.value?.department_id === item.assigned_department_id && item.status != 2 && item.status != 5
}
...
const onClickDeleteButton = (item: Item) => {
    axios.delete('/api/task_assignments/delete', {
        params: {
            id: item.id
        }
    }).then(response => {
...
const doExport = () => {
    // if (computedFormState.value.description) {
        // computedFormState.value.description = encodeURIComponent(computedFormState.value.description)
    // }
    
    exportLoading.value = true
    downloadFile('/api/task_assignments/export', computedFormState.value, errorMessages, t)
    .then(() => {
...
const onRedirect = () => {
    ...
    const newPath = '/issues/department-assigned';
