...
@section('content')
<div class="main-pdf" style="padding:30px">
        ...
        <table class="table-header" style="width:100%;">
                ...
                <td style="width:50%;">
                    <div style="">
                        @if($review->period == 2 || $review->period == 3)
                            <span style="font-weight: bold">VĂN BẢN ĐÁNH GIÁ QUÁ TRÌNH <br><p style="">THỰC HIỆN CÔNG VIỆC<p></span>
                        @elseif($review->period == 4)
                            <span style="font-weight: bold">VĂN BẢN ĐÁNH GIÁ QUÁ TRÌNH <br><p style="">HỌC VIỆC<p></span>
                        @else
                            <span style="font-weight: bold">VĂN BẢN ĐÁNH GIÁ QUÁ TRÌNH <br><p style="">THỬ VIỆC<p></span>
                        @endif  
                    </div>
                </td>
            </tr>
            <tr>
                <td style="width:50%; padding-top:20px; text-align: left;">
                    <span style="">Nhân viên: {{ $employee->fullname }}</span>
                </td>
    ...
    <div class="table-wrapper" style=" margin-bottom:10px">
        <table class="timesheet-table" style="width:100%;">
            <thead>
            <tr>
                ...
                <th style="font-size:12px;width:100px">Tỉ lệ đi muộn</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                ...
                <td>{{$user_table->users[0]->origin_workday > 0 ? number_format(($user_table->users[0]->late_count / $user_table->users[0]->origin_workday) * 100, 2, '.', '') : 0 }}%</td>
    ...
    <div style="">
        @foreach ($violations as $key => $value)
            <span style="color: red; font-size:13px; font-style: italic;">Vi phạm lần {{$key+1}}: {{ $value->description }} ({{$value->time_violation}})</span>
            <br>
        @endforeach
    </div>
    @endif
    ...
    <div style="padding-top: 10px">
        @foreach ($questions as $key1 => $value1)
            ...
            <span style="font-weight: bold">
                {{ $key1+1 }}.{{ $value1->question }}
                @if(in_array($value1->question_id, [17, 24, 33, 48]))
                    @if($value1->type === 0)
                        Member
                    @elseif($value1->type === 0.5)
                        của Mentor
